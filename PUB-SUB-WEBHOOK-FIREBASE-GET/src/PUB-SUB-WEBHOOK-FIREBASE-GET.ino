 ...
 * 
 * For this type of HTTP REQUEST, we will use HTTP GET. We want to GET the data for
 * a particular sensor or for a particular device - be careful the GET responce 
 * may be large. We will use the JsonParserGenerator ...
 * Author: slab
 * Date: 18022021
 ********************************************************************************/
...
void setup() {
  ...
  /* SUBSCRIBE TO FIREBASE JSON RESPONSE */
  Particle.subscribe("hook-response/cslabfirebase-get", firebaseHandler);
...
void loop() { 
  ...
  if( digitalRead(B_TN) == LOW ) {
    ...
    /* FIREBASE REQUEST FOR DATA */
    Particle.publish("cslabfirebase-get", "{\"sensor\":\"9DOF-IMU\"}");
...
void firebaseHandler(const char *topic, const char *data) {
  Serial.println();
  Serial.println("FIREBASE JSON RESPONSE: ");
  Serial.println( data );
}
